...
from app.core.services.threads.thread_workflow_synchronizer import ThreadWorkflowSynchronizer


class CoreDraftService:
    """
    Core service handling draft creation, updating, database persistence, task resolutions,
    thread workflow_status synchronization, and Google Gmail API draft synchronization.
    No dependency on web_services or api layer schemas.
    """
    ...
    async def create_draft(
        ...
        """
        Creates a draft, synchronizes with Gmail API (users().drafts().create),
        persists into public.email_drafts, resolves selected tasks via public.email_draft_resolutions,
        and updates thread workflow_status.
        """
        print(f"[CORE DRAFT LOG 1/6] Validating thread {thread_id} for account {account_id}")
        ...
        print(f"[CORE DRAFT LOG 2/6] Found thread record. Gmail Thread ID: {gmail_thread_id}")
        ...
        if existing_draft_res.data:
            ...
            print(f"[CORE DRAFT UPSERT] Active draft {existing_draft_id} exists for thread. Updating existing draft...")
        ...
        print(f"[CORE DRAFT LOG 3/6] Fetching valid Google OAuth credentials for account {account_id}")
        gmail_client = await self.core_auth.get_authenticated_gmail_client(account_id)

        # 4. Construct base64url MIME email message
        print(f"[CORE DRAFT LOG 4/6] Building base64url MIME message for recipients {recipient_to}")
        mime_msg = EmailMessage()
        mime_msg["To"] = ", ".join(recipient_to)
        mime_msg["Subject"] = subject or thread.get("subject") or ""
        if reply_to_msg_id:
            mime_msg["In-Reply-To"] = reply_to_msg_id
            mime_msg["References"] = reply_to_msg_id

        mime_msg.set_content(body or "")
        ...
        print(f"[CORE DRAFT LOG 5/6] Executing Gmail API users().drafts().create()")
        gmail_draft_id = None
        error_log = None
        try:
            gmail_res = gmail_client.users().drafts().create(userId="me", body=draft_payload).execute()
            gmail_draft_id = gmail_res.get("id")
            print(f"[CORE DRAFT LOG 5/6 SUCCESS] Gmail draft ID created: {gmail_draft_id}")
        except Exception as e:
            error_log = f"Gmail draft sync failed: {str(e)}"
            print(f"[CORE DRAFT WARNING] {error_log}")
            import traceback
            traceback.print_exc()

        # 6. Insert record into public.email_drafts
        print(f"[CORE DRAFT LOG 6/6] Inserting record into public.email_drafts (status='{status}')")
        ...
        if task_ids:
            ...
            try:
                ...
                print(f"[CORE DRAFT WARNING] Failed to persist task resolutions: {ex}")

        draft_data["resolved_task_ids"] = task_ids
        return draft_data
    ...
    async def update_draft(
        ...
        """Updates draft fields in public.email_drafts, syncs to Gmail API, and updates thread workflow_status."""
        ...
        now_iso = datetime.now(timezone.utc).isoformat()
        update_fields: Dict[str, Any] = {"updated_at": now_iso, "status": "draft"}

        if recipient_to is not None:
            update_fields["recipient_to"] = recipient_to
        ...
        if gmail_draft_id:
            try:
                gmail_client = await self.core_auth.get_authenticated_gmail_client(account_id)
                new_recipients = recipient_to if recipient_to is not None else draft.get("recipient_to", [])
                ...
                print(f"[CORE DRAFT UPDATE WARNING] Gmail draft update failed: {e}")
    ...
    async def send_draft(self, user_id: str, account_id: str, draft_id: str) -> Dict[str, Any]:
        ...
        if gmail_draft_id:
            try:
                ...
                print(f"[CORE DRAFT SEND WARNING] Gmail draft send failed: {e}")

        now_iso = datetime.now(timezone.utc).isoformat()
        self.db.table("email_drafts").update({"status": "sent", "updated_at": now_iso}).eq("id", draft_id).execute()

        # Update thread workflow status upon sending
        if thread_id:
            try:
                await self.synchronizer.sync_thread_status(thread_id, account_id)
            except Exception as ex:
                print(f"[CORE DRAFT SEND WARNING] Thread status sync failed: {ex}")
